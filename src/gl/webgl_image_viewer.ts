  ...
  exportAsBMP(): Blob {
    ...
    for (let y = 0; y < outputHeight; y++) {
      for (let x = 0; x < outputWidth; x++) {
        ...
        const r = pixels[srcIdx + 0]; // R

        if (!isNumber(b) || !isNumber(g) || !isNumber(r)) {
          throw new Error('Invalid pixel data');
        }

        bgrPixels[dstIdx + 0] = b; // B
        bgrPixels[dstIdx + 1] = g; // G
        bgrPixels[dstIdx + 2] = r; // R
      }
